...
impl Attrs {
    pub fn new(attrs: &[syn::Attribute], errors: &Ctxt) -> Self {
        ...
        let (doc_title, doc_description, doc_classes) = doc::get_title_desc_class_from_doc(attrs);
        result.title = result.title.or(doc_title);
        result.description = result.description.or(doc_description);
        result.classes = result.classes.or(doc_classes);
        result
    }
    ...
    pub fn as_metadata(&self) -> SchemaMetadata<'_> {
        ...
        SchemaMetadata {
            ...
            classes: self.classes.as_ref().and_then(none_if_empty),
    ...
    fn populate(
        ...
        for meta_item in get_meta_items(attrs, attr_type, errors, ignore_errors) {
            match &meta_item {
                ...
                Meta(NameValue(m)) if m.path.is_ident("classes") => {
                    if let Ok(classes) = get_lit_str(errors, attr_type, "classes", &m.lit) {
                        match self.classes {
                            Some(_) => duplicate_error(m),
                            None => self.classes = Some(classes.value()),
                        }
                    }
                }
