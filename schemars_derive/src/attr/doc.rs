...
pub fn get_title_desc_class_from_doc(
    attrs: &[Attribute],
    ...
    if doc.starts_with('#') {
        ...
        let maybe_class = split.next().and_then(merge_description_lines);
        (none_if_empty(title), maybe_desc, maybe_class)
    } else {
        (
            None,
            merge_description_lines(&doc),
            merge_classes_lines(&doc),
        )
    }
}
...
fn merge_classes_lines(doc: &str) -> Option<String> {
    let class = doc
        .trim()
        .split("\n\n")
        .filter_map(|line| none_if_empty(line.trim().replace('\n', " ")))
